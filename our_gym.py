...
'''
    Action space: (Tuple)
        ...
        action: {0:MOVE_FORWARD, 1:MOVE_BACKWARD, 2:TURN_RIGHT, 3:TURN_LEFT, 4:INCREASE_HEIGHT, 5:DECREASE_HEIGHT, 6:extract_fruit}
    Reward: 
        ACTION_COST = -1
        PICK_NOTHING_PENALTY = -3
        PICK_UNRIPE_PENALTY = -5
        COLLISION_PENALTY = -5
...
# Robot parameters
limit_length = 1  # Maximum manipulator length
limit_height = (4, 8)  # Height range
base_size = 3  # Base size of the robot
base = base_size // 2  # Half base size

goal_fruit = 3  # Number of fruits required to complete the task
...
class State(object):
    ...
    def __init__(self, world0, goals):
        ...
        self.agent_pos, self.agent_past, self.agent_goals, self.goals_ripeness, self.tree_positions = self.scanForAgent()
            ...
            for j in range(self.state.shape[1]):
                for k in range(self.state.shape[2]):
                    ...
                    if (self.goals[i, j, k] != 0):
                        ...
                        if (self.goals[i, j, k] == -1):
                            goals_ripeness.append((i, j, k, -1))
                        elif (self.goals[i, j, k] == 1):
                            goals_ripeness.append((i, j, k, 1))

        assert (agent_pos != (-1, -1, -1) and agent_goals != [])
        assert (len(agent_goals) == len(goals_ripeness))
        assert (agent_pos == agent_past)
        return agent_pos, agent_past, agent_goals, goals_ripeness, tree_positions
    ...
    def getGoal(self):
        # Returns the list of goal positions.
        return self.agent_goals

    def gettree(self):
        # Returns the list of tree positions and their heights.
        return self.tree_positions

    def getripeness(self):
        # Returns the list of the fruit ripeness.
        return self.goals_ripeness
    ...
    returns:
          ...
          0: action executed
         -1: extract executed and pick no fruit
         -2: action executed and reached a goal(pick an unripe fruit)
         -3: out of bounds
         -4: collision with tree
         -5: extend out of length limits
    '''
        ...
        '''
            action:
                {0: MOVE_FORWARD,
                 ...
                 6: EXTRACT_FRUIT}
            '''
        if action == 6:  # EXTRACT_FRUIT
            return self.extract_fruit()
    ...
    def extract_fruit(self):
        '''
        Attempt to extract a fruit at the current end-effector position.

        Returns:
            int: Status of the fruit extraction:
                2: All goals achieved (complete task).
                1: Successfully picked a ripe fruit.
               -1: No fruit to pick at this location.
               -2: Picked an unripe fruit.
        '''
        x, y, z = self.getPos()
        o = self.getOrient()
        eex, eey, eez = self.ee_position(x, y, z, o)
        if (eex, eey, eez) in self.agent_goals:
            goal_status = self.goals[eex, eey, eez]
            if goal_status == 1:
                self.goals[eex, eey, eez] = 0
                self.agent_goals.remove((eex, eey, eez))
                self.goals_ripeness.remove((eex, eey, eez, 1))
                self.total_fruit += 1
                assert (len(self.agent_goals) == len(self.goals_ripeness))
                if self.total_fruit >= goal_fruit or not np.any(self.goals == 1):
                    return 2
                return 1  # Successfully picked a ripe fruit
            elif goal_status == -1:
                self.goals[eex, eey, eez] = 0
                self.agent_goals.remove((eex, eey, eez))
                self.goals_ripeness.remove((eex, eey, eez, -1))
                assert (len(self.agent_goals) == len(self.goals_ripeness))
                return -2  # Picked an unripe fruit
        return -1  # No fruit to extract
...
class FruitPickingEnv(gym.Env):
    ...
    def __init__(self, world0=None, goals0=None, SIZE=(10, 10, 10), tree_height=7, canopy_size=5):
        ...
        self.action_space = spaces.Discrete(7)  # 7 actions as defined
        self.viewer = None
    ...
    # Generate tree and fruit randomly
    def _generateTreesAndGoals(self):
        ...
        for _ in range(random.randint(66, 88)):  # Randomly generate fruits
            ...
            if dx == 0 and dy == 0:
                ...
                ripeness = random.choice(p_ripe_unripe)
                goals[fruit_pos[0], fruit_pos[1], fruit_pos[2]] = ripeness
        return world, goals
    ...
    def step(self, action):
        ...
        if action_status == 2:
            ...
            # The robot tried to pick the fruit but didn't get it
            reward = PICK_NOTHING_PENALTY
        elif action_status == -2:
            # The robot picked up an unripe fruit
            reward = PICK_UNRIPE_PENALTY
    ...
    def _Render(self):
        if not hasattr(self, "client_id"):
            ...
            for goal in goals:
                ...
                if ripe == 1:
                    ...
                    fruit_id = p.loadURDF("models/fruit_yellow.urdf",
                                          position,
                                          p.getQuaternionFromEuler([0, 0, 0]))
        ...
        self.fruit_objects = []  # Clear storage list
        ...
        for goal in goals:
            ...
            if ripe == 1:
                ...
                fruit_id = p.loadURDF("models/fruit_yellow.urdf", position,
                                      p.getQuaternionFromEuler([0, 0, 0]))
            self.fruit_objects.append(fruit_id)
